...
from robot_sim_bringup.platform.adapter import preflight_adapter
from robot_sim_bringup.platform.assertions import evaluate_assertions
from robot_sim_bringup.platform.config import load_platform_validation_case
...
def _start_adapter(runner: Any, adapter: Mapping[str, Any], adapter_dir: Path, logs_dir: Path) -> dict[str, Any]:
    ...
    command = [
        sys.executable,
        "-m",
        "robot_sim_bringup.platform.adapter",
        "--adapter-file",
        str(adapter_file),
    ]
...
def _service_call_action(action: Mapping[str, Any], started: float) -> dict[str, Any]:
    ...
    from robot_sim_bringup.platform.assertions import assign_fields, message_to_dict
...
def _merged_env(extra: Mapping[str, Any]) -> dict[str, str]:
    ...
    source_roots = [
        ...
        Path(__file__).resolve().parents[3] / "robot_sim_scenarios",
    ]
