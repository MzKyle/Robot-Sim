...
class ImuReceiver : public rclcpp::Node
{
public:
  ImuReceiver()
  : rclcpp::Node("imu_receiver")
  {
    ...
    receiver_type_ = declare_parameter<std::string>("receiver_type", "imu");
    sensor_name_ = declare_parameter<std::string>("sensor_name", "imu");

    add_subscription<sensor_msgs::msg::Imu>("imu", imu_topic);
    ...
    timer_ = create_wall_timer(
      ...
      std::bind(&ImuReceiver::publish_health, this));
  }
...
private:
  ...
  void publish_health()
  {
    ...
    for (const auto & item : stats_) {
      ...
      status.hardware_id = "robot_sim_sensor_imu";
      if (stat.count == 0) {
...
int main(int argc, char ** argv)
{
  ...
  rclcpp::spin(std::make_shared<ImuReceiver>());
  rclcpp::shutdown();
  return 0;
}
