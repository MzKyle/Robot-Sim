...
public:
  LidarReceiver()
  : rclcpp::Node("lidar_receiver")
  {
    const auto scan_topic =
      declare_parameter<std::string>("scan_topic", "/scan");
    const auto pointcloud_topic =
      declare_parameter<std::string>("pointcloud_topic", "/lidar/points");
    ...
    receiver_type_ = declare_parameter<std::string>("receiver_type", "lidar");
    sensor_name_ = declare_parameter<std::string>("sensor_name", "lidar");

    add_subscription<sensor_msgs::msg::LaserScan>("scan", scan_topic);
    ...
    timer_ = create_wall_timer(
      ...
      std::bind(&LidarReceiver::publish_health, this));
  }
...
private:
  ...
  void publish_health()
  {
    ...
    for (const auto & item : stats_) {
      ...
      status.hardware_id = "robot_sim_sensor_lidar";
      if (stat.count == 0) {
...
int main(int argc, char ** argv)
{
  ...
  rclcpp::spin(std::make_shared<LidarReceiver>());
  rclcpp::shutdown();
  return 0;
}
