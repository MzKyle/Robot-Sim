...
class DepthReceiver : public rclcpp::Node
{
public:
  DepthReceiver()
  : rclcpp::Node("depth_receiver")
  {
    ...
    const auto pointcloud_topic =
      declare_parameter<std::string>("pointcloud_topic", "/camera/points");
    ...
    receiver_type_ = declare_parameter<std::string>("receiver_type", "depth");
    sensor_name_ = declare_parameter<std::string>("sensor_name", "depth");

    add_subscription<sensor_msgs::msg::Image>("depth_image", depth_image_topic);
    add_subscription<sensor_msgs::msg::CameraInfo>("camera_info", camera_info_topic);
    add_subscription<sensor_msgs::msg::PointCloud2>("pointcloud", pointcloud_topic);
    ...
    timer_ = create_wall_timer(
      ...
      std::bind(&DepthReceiver::publish_health, this));
  }
...
private:
  ...
  void publish_health()
  {
    ...
    for (const auto & item : stats_) {
      ...
      status.hardware_id = "robot_sim_sensor_depth";
      if (stat.count == 0) {
...
int main(int argc, char ** argv)
{
  ...
  rclcpp::spin(std::make_shared<DepthReceiver>());
  rclcpp::shutdown();
  return 0;
}
