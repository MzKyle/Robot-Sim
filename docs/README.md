...
它围绕“相机采集 -> 机器人状态 -> 采集控制 -> 数据落盘 -> 历史检索”这条主线组织代码，同时提供 `robot_sim_*` 仿真平台，方便在没有真实设备时验证 Panda 控制、Gazebo 场景和传感器话题。
...
- 提供 `robot_sim_bringup` 仿真入口，支持 `mock`、`light`、`full` 三档模式和按组传感器开关。
...
flowchart LR
  ...
  subgraph Sim[仿真设备]
    GZ[gz sim 8\nrobot_sim_bringup]
    PANDA[Panda Arm\nxacro spawn]
    GZCTRL[gz_ros2_control\nGazeboSimSystem]
  end

    subgraph Runtime[采集后端]
        BRINGUP[data_collect_bringup]
    SIMBRINGUP[robot_sim_bringup]
        CFG[(src/config/nodemanage.yaml)]
    SIMCFG[(sim_mode + sensor groups)]
    ...
    GZ -->|camera/depth/lidar/imu| COLLECT
    GZCTRL -->|/joint_states /arm_controller| COLLECT
...
| `src/robot_sim_bringup/` | gz sim 8 仿真入口、三档模式和传感器桥接 |
| `src/robot_sim_description/` | Panda xacro、mesh 和 Gazebo 传感器描述 |
...
- [仿真方案](guide/simulation.md) - 了解 gz sim 8 + Panda 三档仿真模式。
