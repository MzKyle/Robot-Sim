...
`robot_sim` 是 ROS2 机器人仿真与验收平台，当前只维护 `schema: 3` 的 profile/scene/case 机器人链路。通用 `schema: 4` ROS2 pipeline 验证已经拆到同级项目 `robot_validation`。

这个项目不是单纯打开 Gazebo 的 demo，也不是生产机器人控制器。它面向工程验收：
回答“机器人模型、控制链、TF、传感器、MoveIt、业务模块是否能稳定跑通”。
...
- [外部项目资产](guide/external-projects.md)：v3 外部机器人仿真包规范。
...
- [配置说明](configuration/settings.md)：v3 robot domain 配置结构。
...
| 路径 | 说明 |
| --- | --- |
| `src/core/robot_sim_bringup/` | robot domain runner、legacy integration、registry、scaffold 和 launch |
| `examples/` | robot arm 内置示例资产 |
