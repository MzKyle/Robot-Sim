...
本工作空间使用 `robot_sim_*` 包族作为唯一仿真入口。它以 Panda 机械臂为底座，打通 gz sim 8、`gz_ros2_control`、`ros_gz_bridge`、MoveIt2、RViz2 和常用传感器话题；旧 `data_collect_sim` 仿真链路已移除。
...
本机 `gz sim 8` 属于 Harmonic；ROS 2 Humble + Harmonic 组合需要源码版 `gz_ros2_control` overlay。apt 版 `ros-humble-gz-ros2-control` 面向 Fortress，不能直接作为 `gz sim 8` 的系统插件使用。

默认启动轻量仿真：Gazebo + `gz_ros2_control/GazeboSimSystem`，关闭传感器、MoveIt2 和 RViz2，适合日常开发：

```bash
ros2 launch robot_sim_bringup sim.launch.py
```

完整仿真会打开全部传感器、MoveIt2 和 RViz2：

```bash
ros2 launch robot_sim_bringup sim.launch.py sim_mode:=full
```

最快的 mock 模式不启动 Gazebo，只用 `mock_components/GenericSystem` 验证 ROS 2 控制链：
...
- `sim.launch.py`：单机仿真入口，支持 `mock`、`light`、`full` 三档模式。
...
控制配置在 `robot_sim_control/config/panda_controllers.yaml`。Gazebo 模式由 `libgz_ros2_control-system.so` 在 Gazebo 内创建 controller manager，默认控制器为：
...
`mock` 模式会在 ROS 侧启动 `ros2_control_node`；`light` 和 `full` 模式使用真正的 Gazebo hardware plugin，不再经过 joint command bridge 适配层。
...
`sim_mode:=full` 默认会启动 MoveIt2 和 RViz2。在 RViz2 中选择 `panda_arm` 后可规划并执行，轨迹会进入：
...
`full` 模式默认启用 gz 原生传感器和 `ros_gz_bridge`，公开标准 ROS 2 话题：
